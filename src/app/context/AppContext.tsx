...
import type { InstitutionFaculty } from "../lib/institution";
import {
  ...
  getAllFaculties, addFaculty as storeAddFaculty, updateFaculty as storeUpdateFaculty, deleteFaculty as storeDeleteFaculty,
} from "../lib/store";
...
interface AppDataContextValue {
  ...
  createFaculty: (name: string, departments: string[], dean?: string) => InstitutionFaculty;
  editFaculty: (id: string, patch: Partial<Pick<InstitutionFaculty, "name" | "dean" | "departments">>) => InstitutionFaculty | null;
...
createFaculty: (name, departments, dean) => {
  const f = storeAddFaculty(name, departments, dean);
  refresh();
  return f;
},
editFaculty: (id, patch) => {
  const f = storeUpdateFaculty(id, patch);
  refresh();
  return f;
},
removeFaculty: (id) => {
  const ok = storeDeleteFaculty(id);
  refresh();
  return ok;
},
