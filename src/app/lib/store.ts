...
import type { InstitutionFaculty } from "./institution";
...
import { INSTITUTION_COURSES, INSTITUTION_LECTURERS } from "./institution";
...
const SEED_COURSES: Course[] = INSTITUTION_COURSES.map(ic => {
  const lecturer = INSTITUTION_LECTURERS.find(l => l.id === ic.lecturerId);
  return {
    id: ic.id,
    code: ic.code,
    title: ic.title,
    ...
    faculty: getFacultyForDepartment(ic.department),
    lecturer: lecturer?.name || "Unassigned",
    lecturerId: ic.lecturerId || undefined,
    level: ic.level,
    semester: ic.semester,
    subjects: buildSubjects(ic.code, ic.title),
  };
});
...
function migrateStore(store: DataStore): boolean {
  ...
  if (store.version < STORE_VERSION) {
    ...
    // Version 8: Add all comprehensive courses from institution data
    if (store.version < 8) {
      const existingCodes = new Set(store.courses.map(c => c.code));
      const newCourses = INSTITUTION_COURSES
        .filter(ic => !existingCodes.has(ic.code))
        .map(ic => {
          const lecturer = INSTITUTION_LECTURERS.find(l => l.id === ic.lecturerId);
          return {
            id: ic.id,
...
export function getAllFaculties(): InstitutionFaculty[] {
  const store = readStore();
  return store.faculties;
}

export function addFaculty(name: string, departments: string[], dean?: string): InstitutionFaculty {
  const store = readStore();
  if (store.faculties.some(f => f.name.toLowerCase() === name.toLowerCase())) {
    throw new Error(`Faculty "${name}" already exists.`);
  }
  const faculty: InstitutionFaculty = {
    id: uid("F"),
    name,
    dean: dean || "Unassigned",
    departments,
  };
  store.faculties.push(faculty);
  writeStore(store);
  notifyRegistrars("Faculty Created", `${name} faculty was added with ${departments.length} department(s).`, "system", "course-mgmt");
  return faculty;
}

export function updateFaculty(id: string, patch: Partial<Pick<InstitutionFaculty, "name" | "dean" | "departments">>): InstitutionFaculty | null {
  const store = readStore();
  const idx = store.faculties.findIndex(f => f.id === id);
  if (idx < 0) return null;
  const faculty = store.faculties[idx];
  if (patch.name) faculty.name = patch.name;
  if (patch.dean !== undefined) faculty.dean = patch.dean;
  if (patch.departments) faculty.departments = patch.departments;
  writeStore(store);
  return faculty;
}
